...
import org.fujionclinical.fhir.api.r5.common.AttachmentTransform;
...
public class DocumentWrapper extends BaseResourceWrapper<DocumentReference> implements IDocument {
    ...
    private static class RelatedWrapper extends AbstractWrapper<DocumentReferenceRelatesToComponent> implements IRelatedDocument {
        ...
        public void setRelationship(DocumentRelationship relationship) {
            getWrapped().setCode(FhirUtil.convertEnum(relationship, Enumerations.DocumentRelationshipType.class));
        }
    ...
    public DateTimeWrapper getCreationDate() {
        return FhirUtil.convertDate(getWrapped().getDate());
    }

    @Override
    public void setCreationDate(DateTimeWrapper creationDate) {
        getWrapped().setDate(FhirUtil.convertDate(creationDate));
    }
    ...
    public void setCompositionStatus(CompositionStatus status) {
        getWrapped().setDocStatus(FhirUtil.convertEnum(status, Enumerations.CompositionStatus.class));
    }
