...
public class BaseService extends BaseFhirService<Patient, Identifier, Reference> {
    ...
    public <T extends IBaseResource> T createResource(T resource) {
        ...
        return FhirUtilR5.processMethodOutcome(outcome, resource);
    }
    ...
    public List<IBaseResource> everything(IBaseResource resource) {
        ...
        return FhirUtilR5.getEntries(bundle);
    }
    ...
    public <T extends IBaseResource> List<T> searchResourcesByIdentifier(
            ...
            Class<T> clazz,
            int maxCount) {
        return searchResourcesByIdentifier(FhirUtilR5.createIdentifier(system, value), clazz, maxCount);
    }
    ...
    public <T extends IBaseResource> List<T> searchResourcesByIdentifier(
        ...
        return FhirUtilR5.getEntries(bundle, clazz);
    }
    ...
    public List<IBaseResource> searchResourcesByTag(
        ...
        return FhirUtilR5.getEntries(bundle, null, null);
    }
    ...
    public <T extends IBaseResource> List<T> searchResourcesForPatient(
            ...
            int maxCount) {
        Bundle bundle = getClient().search().forResource(clazz).count(maxCount)
                .where(Constants.PARAM_PATIENT.hasId(FhirUtilR5.getResourceIdPath(patient))).returnBundle(Bundle.class).execute();
    ...
    public <T extends IBaseResource> T updateResource(T resource) {
        MethodOutcome outcome = getClient().update().resource(FhirUtilR5.stripVersion(resource)).execute();
        return FhirUtilR5.processMethodOutcome(outcome, resource);
    }

    @Override
    protected void validateClient() {
        FhirUtilR5.assertFhirVersion(getClient());
    }
