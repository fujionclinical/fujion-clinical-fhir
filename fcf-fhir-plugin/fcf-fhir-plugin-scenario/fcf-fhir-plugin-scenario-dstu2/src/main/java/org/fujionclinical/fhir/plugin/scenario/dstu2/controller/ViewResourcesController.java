...
public class ViewResourcesController extends FrameworkController {
    ...
    private static IComponentRenderer<Row, IBaseResource> resourceRenderer = (resource) -> {
        Row row = new Row();
        row.addChild(new Cell(FhirUtilDstu2.getResourceIdPath(resource)));
    ...
    private void onClick$btnDelete() {
        ...
        DialogUtil.confirm("Delete " + FhirUtilDstu2.getResourceIdPath(resource, true) + "?", "Delete Resource", (confirm) -> {
            if (confirm) {
                try {
